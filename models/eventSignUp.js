...
class EventSignUp {
    constructor(signup_id, firstname, lastname, email, phone_number, signup_date) {
    ...
    static async createEventSignUp(eventSignUpData, accountId) {
        try {
          const connection = await sql.connect(dbConfig);
          const sqlQuery = `
            INSERT INTO EventSignUp (event_id, account_id, firstname, lastname, email, phone_number, signup_date)
            VALUES (@eventId, @accountId, @firstname, @lastname, @email, @phoneNumber, @signupDate);
          ...
          const newEventSignUp = new EventSignUp(insertedId, eventSignUpData.event_id, accountId, eventSignUpData.firstname, eventSignUpData.lastname, eventSignUpData.email, eventSignUpData.phone_number, eventSignUpData.signup_date);
